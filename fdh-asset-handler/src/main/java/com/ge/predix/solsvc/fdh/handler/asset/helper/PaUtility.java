...
public class PaUtility
{
    ...
    public static boolean isModelAttributeField(String fieldIdString)
    {
        ...
        if ( !fieldIdString.startsWith("/") ) localString = "/" + fieldIdString.substring(0);
    ...
    {
        String localString = fieldIdString;
        if ( !fieldIdString.startsWith("/") ) localString = "/" + fieldIdString.substring(1);
        ...
        else if ( localString.split("/").length == 3 )
        {
            ...
            throw new UnsupportedOperationException(
                    "unexpected number of / slash delimiters in fieldIdString=" + localString);
    }
    ...
    public static FieldModel getFieldModel(String field)
    {
        ...
        if ( !field.startsWith("/") ) localString = "/" + field;

        StringTokenizer t = new StringTokenizer(localString, "/");
        if ( !t.hasMoreTokens() ) {
            throw new UnsupportedOperationException("we need to at least know the model type, e.g. /asset, /wing, /jet-engine.  Please add that to the request.");
        }
