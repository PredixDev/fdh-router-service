...
import com.ge.fdh.asset.data.AttributeHandlerInterface;
import com.ge.fdh.asset.helper.ErrorCodes;
 ...
 * - Retrieve the asset using AssetSelector
 * - Get the requested data type, engineering unit and selection filters
...
public class GetFieldDataHandlerImpl extends AbstractFdhRequestProcessor
        implements GetFieldDataHandler
{
    private static final Logger               log = LoggerFactory.getLogger(GetFieldDataHandlerImpl.class);

    @Autowired
    private FieldTypeSelectionFilterProcessor fieldTypeSelectionFilterProcessor;
    @Autowired
    private FieldSelectionFilterProcessor     fieldSelectionFilterProessor;
    @Autowired
     ...
     * (non-Javadoc)
     * @see
     * com.ge.fdh.asset.processor.IGetFieldDataProcessor#processRequest(com.ge.dsp.pm.fielddatahandler.entity.getfielddata.GetFieldDataRequest)
     */
    @Override
    public GetFieldDataResult getFieldData(GetFieldDataRequest getFieldDataRequest, Map<Integer, Model> modelLookupMap,
            List<Header> headers, String httpMethod)
    {
        ...
        // keep a map of AssetSelector to Asset for performance
        try
        {
            this.getFieldDataValidator.validate(getFieldDataRequest, getFieldDataResult);
            ...
            List<Header> headers)
    {
        ...
        {
            // FieldSelectionFilter looks a lot like a sql where clause
            this.fieldSelectionFilterProessor.validateFieldSelectionFilter(fieldDataCriteria);
            return this.fieldSelectionFilterProessor.retrieveModels(fieldDataCriteria, headers);
        }
        else if ( fieldDataCriteria.getSelectionFilter() instanceof FieldTypeSelectionFilter )
        {
            // FieldTypeSelectionFilter used for getting Field Definitions
            this.fieldTypeSelectionFilterProcessor
                    .validateFieldTypeSelectionFilter((FieldTypeSelectionFilter) fieldDataCriteria.getSelectionFilter());
            return this.fieldTypeSelectionFilterProcessor.retrieveModels(fieldDataCriteria, headers);
        }
