...
public class PaUtility
{
     ...
     * @return -
     */
    public static boolean isAssetOrMeterOrMeterExtAttributeField(String fieldIdString)
    {
        if ( fieldIdString.toLowerCase().startsWith("/asset/") || fieldIdString.toLowerCase().startsWith("asset/")
                || fieldIdString.toLowerCase().startsWith("meter/")
                || fieldIdString.toLowerCase().startsWith("/asset/meter/meterattribute/")
                || fieldIdString.toLowerCase().startsWith("meterext/") )
        {
            return true;
        }
        return false;
    ...
    {
        if ( fieldIdString.startsWith("/asset/meter/meterattribute") || fieldIdString.startsWith("meterext") )
        {
            return true;
        }
        return false;
    ...
    {
        if ( fieldIdString.startsWith("/meter") || fieldIdString.startsWith("meter") )
        {
            return true;
        }
        return false;
     ...
     * @return -
     */
    public static String getMeterKey(String fieldIdString)
    {
        String localString = fieldIdString;
        if ( !fieldIdString.startsWith("/"))
            localString = "/" + fieldIdString.substring(1);
        if ( localString.split("/").length == 2 )
        {
            ...
            String meterKey = localString.substring(separatorIndex + 1, localString.length());
            return meterKey.trim();
        }
        else if ( localString.split("/").length == 3 )
        {
            int separatorIndex1 = localString.indexOf('/');
            int separatorIndex2 = localString.lastIndexOf('/');
            String meterKey = localString.substring(separatorIndex1 + 1, separatorIndex2).trim();
            return meterKey.trim();
        }
        else
     ...
     * @return -
     */
    public static String getMeterKeyAttribute(String fieldIdString)
    {
        if ( fieldIdString.split("/").length == 3 )
        {
            int separatorIndex1 = fieldIdString.lastIndexOf('/');
            String meterKey = fieldIdString.substring(separatorIndex1 + 1).trim();
            return meterKey.trim();
        }
        return null;
    }
