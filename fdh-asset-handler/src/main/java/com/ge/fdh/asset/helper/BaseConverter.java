...
abstract public class BaseConverter {
    ...
    protected void copyBaseDataEventProperties(DataEvent src, DataEvent dest,
			Class toClazz) throws OsacbmConversionException {
		if (src == null) {
			throw new OsacbmConversionException(
					"src is null for base properties copy");
		}
		if (dest == null) {
			throw new OsacbmConversionException(
					"dest is null for base properties copy");
		}
		...
		if (Utils.isOsaSingleton(toClazz) && this.xAxisDeltas != null) {
			...
			if (!toClazz.equals(DMVector.class)) {
				...
				Utils.addToTime(dest.getTime(), this.xAxisStart
						+ sum(this.xAxisDeltas));
			}
		}
		...
		if (qualityValues != null) {
			if (Utils.isOsaSingleton(dest.getClass())) {
				Utils.setQuality(dest,
						qualityValues.get(qualityValues.size() - 1));
			} else {
	 ...
	 *             conversion errors
	 */
	abstract public DataEvent convertTo(Class clazz)
			throws OsacbmConversionException;
