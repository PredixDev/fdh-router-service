...
/*import com.ge.dsp.pm.ext.entity.osa.sequenceselectionfilter.SequenceSelectionFilter;
 import com.ge.dsp.pm.ext.entity.osa.timeselectionfilter.TimeSelectionFilter;*/
...
public class Utils
{
            ...
            throws OsacbmConversionException
    {
        ...
        {
            throw new OsacbmConversionException("Error converting the time to a double.", e); //$NON-NLS-1$
        }
            ...
            if ( osaTime.getTime() != null )
            {
                ...
                catch (Exception e)
                {
                    throw new RuntimeException("Unable to parse time : " + osaTime.getTime() //$NON-NLS-1$
                            + " expected date format : " + " for  time type " //$NON-NLS-1$ //$NON-NLS-2$
                            + OsacbmTimeType.OSACBM_TIME_MIMOSA.toString());
                }
                if ( d == null )
                {
                    throw new RuntimeException("Unable to parse time : " + osaTime.getTime() //$NON-NLS-1$
     ...
     */
    /**
     * returns values in varying units - find all uses and deprecate
     * 
     * @param osaTime
     * @return
     * @throws ParseException
            ...
            throws ParseException
    {
        ...
        {
            Date d = getIsoFormat().parse(osaTime.getTime());
    ...
    public static double getEndTimeFromDataEvent(DataEvent dataEvent)
    {
        ...
        try
        {
            ...
            else if ( dataEvent instanceof DMDataSeq )
            {
                ...
                return getTimeAtIndex(tDataSeq.getXAxisDeltas(), tDataSeq.getXAxisStart(), tDataSeq.getXAxisDeltas()
                        .size() - 1);
            }
            ...
            else if ( dataEvent instanceof DAValueWaveform )
            {
                ...
                return ((DoubleValue) tDataSeq.getXAxisStart()).getValue()
                        + (((DoubleValue) tDataSeq.getXAxisDelta()).getValue() * ((DblArrayValue) tDataSeq.getValues())
                                .getValues().size());
            }
    ...
    /**
     * This method creates the ErrorInfo object which is purely an ordered
     * List<Parameter>.
     * 
     * @param contextId
     *            context identifier
     ...
     * @param detailDesc
     *            detailed description of erro
     * @return ErrorDataEvent = List<ErrorInfo>
     * @see ErrorInfo = List<Parameter>
     */
    ...
    /**
     * determine if the selection filter is compatible with an osa data type. At
     * this point the logid only verifies that we have a SequenceSelectionFilter
     * or TimeSelectionFilter and that the datatype is one of da_waveform,
     ...
     * @return -
     */
    public static boolean isCompatibleSelectionFilters(OsacbmDataType dataType, List<SelectionFilter> selectionFilter)
    {
        if ( selectionFilter != null && selectionFilter.size() > 0 )
        {
            SelectionFilter selected = selectionFilter.get(0);
            if ( (dataType.equals(OsacbmDataType.DA_VALUE_DATA_SEQ)
                    || dataType.equals(OsacbmDataType.DA_VALUE_WAVEFORM) || dataType.equals(OsacbmDataType.DA_WAVEFORM)
                    || dataType.equals(OsacbmDataType.DA_DATA_SEQ) || dataType.equals(OsacbmDataType.DM_DATA_SEQ))
            /* && (selected instanceof SequenceSelectionFilter || selected instanceof TimeSelectionFilter) */)
            {
                return true;
            }
        }
     ...
     * wrapper.
     * We have implementation of this funcationality in platform-utils we just
     * need to consolidate these two classes.
     */
    ...
    public static int findDataPointIndexAtTime(DataEvent sourceData, BigInteger timeBinary)
    {
        ...
        {
            double deltaFromStart = (timeBinary.doubleValue() - (((DAWaveform) sourceData).getXAxisStart() + ((DAWaveform) sourceData)
                    .getXAxisDelta()));
            if ( deltaFromStart <= 0 )
            {
                return 0;
        ...
        else
        {
            double deltaFromStart = timeBinary.doubleValue()
                    - (((DoubleValue) ((DAValueWaveform) sourceData).getXAxisStart()).getValue() + ((DoubleValue) ((DAValueWaveform) sourceData)
                            .getXAxisDelta()).getValue());
            if ( deltaFromStart <= 0 )
            {
                return 0;
            }
            return new Long(Math.round(deltaFromStart
                    / ((DoubleValue) ((DAValueWaveform) sourceData).getXAxisDelta()).getValue())).intValue();
            ...
            throws OsacbmConversionException
    {
        ...
        {
            throw new OsacbmConversionException("Error getting times as a double list.", e); //$NON-NLS-1$
        }
    ...
    public static Double[] extractDateArrayAsDoubleFromDataEvent(DataEvent dataEvent)
    {
        ...
        {
            throw new OsacbmConversionException(e.getMessage(), e);
        }
    ...
    public static Double getQualityAsDouble(DataEvent dataEvent)
    {
        if ( !((dataEvent instanceof DMReal) || (dataEvent instanceof DAReal) || (dataEvent instanceof DMInt)
                || (dataEvent instanceof DAInt) || (dataEvent instanceof DMBool) || (dataEvent instanceof DABool) || (dataEvent instanceof DAString)) )
        {
    ...
    public static List<Double> getQualityAsList(DataEvent dataEvent)
    {
        ...
        if ( !((dataEvent instanceof DMDataSeq) || (dataEvent instanceof DADataSeq)
                || (dataEvent instanceof DAWaveform) || (dataEvent instanceof DAValueDataSeq) || (dataEvent instanceof DAValueWaveform)) )
        {
            throw new RuntimeException(dataEvent.toString() + ErrorCodes.FRAME111+ "Get list of quality values for data value"); //$NON-NLS-1$
        }
    ...
    public static void setQualityList(DataEvent dataEvent, List<Double> qualityValues)
    {
        ...
        List<Object> values = Utils.getValuesList(dataEvent);
        if ( values == null )
        {
            throw new RuntimeException(dataEvent.toString() + ErrorCodes.FRAME111 +
